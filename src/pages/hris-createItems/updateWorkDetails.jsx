...
useEffect(()=>{
    axios.get('http://localhost:4000/getWorkDetails/'+prollId,{payrollId,department,division,payGroup,jobGroup,pensionScheme,firstAppointment,currentAppointment,deployment,subcounty,ward,dutyStation,salaryScalePoint,incrementalMonth})
    .then(res => {
        console.log(res);
...
function handleSubmit(e) {
    e.preventDefault();
    axios.put('http://localhost:4000/updateWorkDetails/'+prollId,{payrollId,department,division,payGroup,jobGroup,pensionScheme,firstAppointment,currentAppointment,deployment,subcounty,ward,dutyStation,salaryScalePoint,incrementalMonth})
    .then(res=>{
        console.log(res);
        navigate('/employees');
    }).catch(err=>console.log(err));
